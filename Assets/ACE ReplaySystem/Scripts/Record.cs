...
using UnityEngine;
using TMPro; // Tambahkan ini untuk mendukung TextMeshPro
...
public class Record : MonoBehaviour
{
    ...
    void RecordAudio(Frame frame)
    {
        if (audioSource != null)
        {
            bool previousState = audioPlay; // Simpan status sebelumnya

            // Pastikan audio sedang dimainkan sebelum merekam
            ...
            // Jika audio mulai diputar, simpan waktu mulai
            float startTime = audioSource.isPlaying ? audioSource.time : -1f; // -1 jika audio belum dimainkan

            // Debugging untuk memastikan status audio
            //Debug.Log($"Recording Audio - Playing: {audioPlay}, Start Time: {startTime}, Previous State: {previousState}");

            // Simpan data hanya jika audio benar-benar dimainkan
            if (audioPlay || previousState)
            {
                frame.SetAudioData(new AudioData(audioPlay, startTime, audioSource.pitch, audioSource.volume,
                                                 audioSource.panStereo, audioSource.spatialBlend,
